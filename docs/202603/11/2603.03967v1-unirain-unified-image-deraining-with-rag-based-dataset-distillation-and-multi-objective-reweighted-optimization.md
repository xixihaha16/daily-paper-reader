...
## 摘要
尽管图像去雨领域已取得显著进展，但我们注意到大多数现有方法通常仅针对特定类型的雨水退化而开发，难以泛化到各种现实世界的降雨场景。如何在通用框架内有效地对不同的雨水退化进行建模，对于现实世界的图像去雨至关重要。在本文中，我们提出了 UniRain，这是一个有效的统一图像去雨框架，能够修复在白天和夜间条件下受雨痕和雨滴退化的图像。为了更好地增强统一模型的泛化能力，我们构建了一个基于智能检索增强生成（RAG）的数据集蒸馏流水线，从所有公开的去雨数据集中筛选高质量训练样本，以实现更好的混合训练。此外，我们将一种简单而有效的多目标重加权优化策略融入到非对称混合专家（MoE）架构中，以促进一致的性能并提高在不同场景下的鲁棒性。大量实验表明，我们的框架在所提出的基准测试和多个公开数据集上，其表现优于目前最先进的模型。
