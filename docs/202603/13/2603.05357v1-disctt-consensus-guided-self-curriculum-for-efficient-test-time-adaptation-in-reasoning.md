...
tldr: 针对大语言模型推理任务中测试时自适应（TTA）策略单一导致的不稳定和低效问题，本文提出DiSCTT框架。该框架利用采样轨迹的一致性来评估实例难度，对高一致性样本采用伪标签监督微调，对低一致性样本采用共识正则化的强化学习。实验证明，DiSCTT在多个推理基准上显著提升了准确率，并大幅降低了计算开销，实现了更稳定高效的推理优化。
selection_source: fresh_fetch
motivation: 现有的测试时自适应方法对所有输入采用统一优化目标，难以应对不同难度的推理问题，导致效率低下且不稳定。
method: 提出一种基于共识引导的自课程学习框架，根据采样轨迹的一致性动态分配监督微调或强化学习优化策略。
result: 在数学和通用推理基准测试中，该方法在提高准确率的同时，显著减少了计算资源消耗和训练时间。
conclusion: 显式考虑实例难度和不确定性是实现稳定、高效且有效的推理模型测试时自适应的关键。
---

## 摘要
测试时自适应（Test-time adaptation）为在无需额外监督的情况下提升大语言模型的推理性能提供了一条极具前景的途径，但现有方法通常对所有输入应用统一的优化目标，导致在异构推理问题上的自适应效率低下或不稳定。我们提出了 DiSCTT，这是一个感知难度且由共识引导的自课程框架，它根据从采样推理轨迹的一致性中估计出的实例级认知不确定性，动态分配测试时优化策略。对于高共识的输入，通过使用多数一致的解作为伪标签进行监督微调来巩固；而对于低共识的输入，则通过强化学习进行优化，并采用共识正则化目标，在相关性约束下鼓励多样性。在广泛的数学和通用推理基准测试中，DiSCTT 一致优于强大的测试时自适应基准模型，在实现更高准确率的同时降低了方差，并显著减少了计算量和实际训练时间。这些结果表明，显式地考虑实例难度和不确定性，能够为推理模型实现更稳定、更高效且更有效的测试时自适应。
