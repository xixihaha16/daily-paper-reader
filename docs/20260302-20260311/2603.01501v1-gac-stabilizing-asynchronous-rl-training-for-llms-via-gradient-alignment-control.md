...
## 摘要
异步执行对于将强化学习（RL）扩展到现代大模型工作负载（包括大语言模型和 AI 智能体）至关重要，但它会从根本上改变 RL 的优化行为。虽然先前关于异步 RL 的工作主要集中在训练吞吐量和分布修正上，但我们表明，将异步盲目应用于策略梯度更新会诱发性质截然不同的训练动力学，并导致严重的训练不稳定。通过系统的实证和理论分析，我们确定了这种不稳定性的一项关键特征：与同步训练中观察到的近乎正交的更新相比，异步训练在连续策略梯度之间表现出持续的高余弦相似度。这种陈旧对齐梯度效应放大了相关更新，并增加了过冲（overshooting）和发散的风险。基于这一观察，我们提出了梯度对齐控制（GAC），这是一种简单的动力学感知稳定方法，通过梯度投影调节异步 RL 在陈旧对齐方向上的进展。我们在有限陈旧度下建立了收敛性保证，并从实证上证明了 GAC 能够恢复稳定的同策略（on-policy）训练动力学，即使在高陈旧度下也能达到与同步基准相当的效果。
